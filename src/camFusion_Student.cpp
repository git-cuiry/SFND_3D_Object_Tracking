...
void show3DObjects(const DataFrame& frame, cv::Size worldSize, cv::Mat image, int index, cv::Mat& P_rect_xx, cv::Mat& R_rect_xx, cv::Mat& RT, bool bWait)
{
	...
	auto visImg = image.clone();

	for (auto bb : frame.bbMatches) {
		const auto& boundingBox = frame.boundingBoxes[bb.second];
		// create randomized color for current 3D object
		...
		for (const auto& lidarPoint : boundingBox.lidarPoints)
		{
			...
			// draw individual point
			cv::circle(topviewImg, cv::Point(x, y), 4, boundingBox.color, -1);
		}

		// draw enclosing rectangle
		if (boundingBox.lidarPoints.size()) {
			cv::rectangle(topviewImg, cv::Point(left, top), cv::Point(right, bottom), boundingBox.color, 2);
			showLidarImgOverlay(visImg, boundingBox.lidarPoints, P_rect_xx, R_rect_xx, RT, &visImg, &boundingBox.color);
	...
	visImg.copyTo(finalImage(cv::Rect(imageSize.width, 0, image.cols, image.rows)));

	for (const auto& bb : frame.boundingBoxes)
	{
	...
	cv::putText(finalImage, cv::format("Frame: %d", index), cv::Point2d(30 + imageSize.width, 30), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 0, 255));
...
void clusterKptMatchesWithROI(const BoundingBox& prevBoundingBox, BoundingBox& currBoundingBox, std::vector<cv::KeyPoint>& kptsPrev, std::vector<cv::KeyPoint>& kptsCurr, std::vector<cv::DMatch>& kptMatches)
{
	...
	if (0 == medianDistanceRatios.size())
		return;
	...
	currBoundingBox.kptMatches.clear();
	...
	std::vector<cv::DMatch> kptMatches, double frameRate, double& TTC, cv::Mat* visImg)
{
	...
	auto medianDistRatio{ 0.0 };
	std::sort(distRatios.begin(), distRatios.end());
	if (0 == distRatios.size() % 2)
